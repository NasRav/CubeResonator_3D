...
double Explicit::d_dx(std::vector<std::vector<std::vector<double>>> f, int i, int j, int k)
{
	return (f[i][j][k] - f[i - 1][j][k]) / _dx;
}

double Explicit::d_dy(std::vector<std::vector<std::vector<double>>> f, int i, int j, int k)
{
	return (f[i][j][k] - f[i][j - 1][k]) / _dy;
}

double Explicit::d_dz(std::vector<std::vector<std::vector<double>>> f, int i, int j, int k)
{
...
{
	return 2 * ( pow(d_dx(_u, i, j, k), 2) + pow(d_dy(_v, i, j, k), 2) + pow(d_dz(_w, i, j, k), 2)
		+ 0.5 * pow(d_dy(_u, i, j, k) + d_dx(_v, i, j, k), 2)
		+ 0.5 * pow(d_dz(_v, i, j, k) + d_dy(_w, i, j, k), 2)
		+ 0.5 * pow(d_dx(_w, i, j, k) + d_dz(_u, i, j, k), 2) )
		- 2 / 3 * pow(d_dx(_u, i, j, k) + d_dy(_v, i, j, k) + d_dz(_w, i, j, k), 2);
}
